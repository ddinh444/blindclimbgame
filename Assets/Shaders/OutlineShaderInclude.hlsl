...
void RobertsCrossNormalBasedOutlines(float2 UV, float2 maxPxOffset, out float Out){
    ...
    Out = step(0.5,sqrt(dot(diff1, diff1) + dot(diff2,diff2)));
}
...
void CalculateOutlinesWithAudioSourceMasks_float(float3 wsPos, float4 UV, float2 maxPxOffset, out float Out){
    ...
    AudioSourceData src12 = _AudioSources[12];

    float3 diff12 = wsPos - src12.position;
    float dist12 = length(diff12);
    float ringMask = step(3.49, dist12) * step(dist12, 3.5);

    Out = accumEchoStrength * (normalOut + depthOut) + ringMask;
}